...
To answer the second question, we can look at an example of an empirically calculated formula for the grain velocity. [Hirashita and Aoyama (2019)](https://ui.adsabs.harvard.edu/abs/2019MNRAS.482.2555H/abstract) give the following:
...
where $\mathcal{M}$ is the Mach number of the largest eddy velocity in the turbulent ISM (which they practically use as an adjusting parameter for the grain velocity) and $s$ is the material density of dust. The idea behind this formula is that under normal circumstances, dust spatially coincident wtih gas due to magnetic fields and moves along with dust due to gas drag forces acting on dust. So, this formula is derived by equating the gas drag timescale and the turnover time due to turbulent velocities in the ISM. This formula captures the qualitative properties of grain velocities: larger grains have larger velocities because of their larger Larmor radii, grain velocities are larger in a higher temperature environment because of the higher characteristic velocity, grain velocities are larger in a less dense medium because of weaker gas drag, and more massive grains have higher velocities because of their larger inertia. According to [Hirashita and Aoyama (2019)](https://ui.adsabs.harvard.edu/abs/2019MNRAS.482.2555H/abstract), in the diffuse ISM ($n_\textrm{H}\sim0.1--0.3~\textrm{cm}^{-3}$) large grains have a velocity of $\sim10~\textrm{km}\,\textrm{s}^{-1}$.

So, in short, it's hard to get a precise answer to this question, but this can give us an idea of typical relative velocities in a continuous medium.
